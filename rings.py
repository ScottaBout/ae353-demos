...
class SimpleClient:
    ...
    def move_through_ring(self, p1, ring, yaw, dt):
        print(f'Move from current position {p1} through ring located at {ring}\n')
        p1 = np.array([p1[0]+0.05, p1[1], p1[2]])
        pre_ring = np.array([ring[0]-0.05, ring[1], ring[2]])
        post_ring = np.array([ring[0]+0.05, ring[1], ring[2]])

        self.move_smooth(p1, pre_ring, yaw, dt-1.)
        self.move_smooth(pre_ring, post_ring, yaw, 1.)
        print('')
...
if __name__ == '__main__':
    ...
    client.move(0., 0., 0.3, 0., 2.0)

    # Move through rings
    client.move_through_ring([0., 0., 0.3], ring1, 0., 5.)
    client.move_through_ring(ring1, ring2, 0., 5.)
    client.move_through_ring(ring2, ring3, 0., 5.)
    client.move_smooth([ring3, [2., 0., 0.3], 0., 5.])

    # Prepare to land
    client.move(2., 0., 0.15, 0., 1.)
